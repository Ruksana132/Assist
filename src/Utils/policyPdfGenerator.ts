...
export const generatePolicyPDF = (record: VehicleServiceRecord): void => {
  ...
  const startDate = new Date(record.vehicleSaleDate);
  const expiryDate = addYears(startDate, 2);
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(30, 58, 138);
  doc.text('CERTIFICATE-CUM-TAX INVOICE', 105, yPos, { align: 'center' });
  yPos += 15;
  ...
  const invoiceDetails = [
    ...
    ['Certificate Print Date:', format(new Date(), 'dd MMM yyyy HH:mm')],
  ];
  ...
  doc.setFontSize(12);
  doc.setFont('helvetica', 'bold');
  doc.text('Original Invoice / Customer Copy', 105, yPos, { align: 'center' });
  yPos += 15;
  ...
  const vehicleDetails = [
    ['Model:', `${record.model} ${record.variant}`],
